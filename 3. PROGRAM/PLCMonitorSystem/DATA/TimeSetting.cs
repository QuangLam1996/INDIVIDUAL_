...
namespace PLCMonitorSystem
{
    public class TimeSetting
    {
        ...
        public Delay[] _arrDelay = new Delay[Enum.GetValues(typeof(eCount)).Length];

        public void IndicatorTime()
        {       
            for (int i = 0; i < _arrDelay.Length; i++)
            {
                _arrDelay[i] = new Delay(Enum.GetValues(typeof(eDelay)).Length, Enum.GetValues(typeof(eDelay)).Length);
            }
